...
const LatestNewsWidget = ({ data }) => {
  ...
  const latestNews = data?.latestNews.slice(0, 3);

  if (latestNews.length === 0) return null;

  return (
    <div className="border border-top-0">
      <h6 className="p-2 main-title">{t("latest_news.mainTitle")}</h6>
          ...
          return (
            <div key={article.articleID} className="p-1 px-2 border-bottom">
              <span
                ...
                onClick={() =>
                  (window.location.href = `/${currentLanguage}/latest-news/${article.articleID}`)
                }
              >
                {article.title}
              </span>
